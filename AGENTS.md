...
- Du bekommst pro Aufgabe genau: Quell-DDL (+ gleichnamige `.md`-Fachnotiz
  falls vorhanden), direkte Vorgänger-Interfaces, gematchte
  `memory/rules/*.md`, letzte Fehlerzeile. Nichts sonst anfordern.
...
- Neue Regel nach behobenem Fehler: `memory/rules/<code>.md`, ≤ 5 Zeilen,
  generalisiert. Vor jedem eigenen Fix-Versuch: `rg <fehlertext oder
  EXEC-Aufruf> skills/` — u.a. Exasol-Fallstricke, Berichtszeitraum-Formel.
...
- **Ausnahme Klasse A:** zeigt G3 einen Content-Fehler in einer generierten
  Klasse-A-Datei (z.B. `tf_pd_fa.sql`): fixen, Modellname zusätzlich in
  `dbt/models/qwen_owned.txt` eintragen und mitcommitten — sonst verwirft
  der nächste `render-a`/`make gate` den Fix wieder.
